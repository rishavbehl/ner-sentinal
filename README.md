...
| **Joblib** | `>=1.3` | Multi-threaded memory-mapped model serialization and persistence |
...
| **Pipeline Runner** | Deterministic, seed-locked dataset generation and cascaded model training via `scripts/pipeline.py`. |
...
│                                                                        │
│   [85 Nodes] ──> [Weather / API-7d] ──> [Model 1: Risk Classifier]    │
│   [98 Roads] ──> [Slope / Exposure] ──> [Model 2: Segment Delay]      │
│         │                                        │                     │
│         └───> [Cascaded Router] <────────────────┘                     │
│                     │                                                  │
│         ┌───────────┴───────────┐                                      │
│         ▼                       ▼                                      │
│   [Model 3: Route Delay]   [Chokepoint / DAI Index]                    │
│         │                       │                                      │
│         ▼                       ▼                                      │
│   [Control Room Dashboard] [Offline Field PWA] [Live GPS Tracking]     │
└────────────────────────────────────────────────────────────────────────┘
```

1. **Live Network Risk Surface:** 98 highway segments continuously scored into calibrated `safe`, `risky`, or `blocked` states.
2. **Cascaded Route Delay Engine:** Captures queue cascades and night hill restrictions rather than naive linear summation.
3. **Chokepoint & Resilience Analytics:** Simulates complete segment closures to identify structural single-points-of-failure and quantify isolated populations.
4. **District Accessibility Index (DAI 0–100):** Objective 5-pillar composite scoring for state and district logistics isolation.
5. **72-Hour Departure-Window Optimizer:** Scans upcoming weather horizons to recommend holding vs. dispatching convoys.
6. **Counterfactual What-If Simulator:** Real-time parameter overrides (e.g. +50mm rain, cloudburst presets) for crisis stress-testing.
7. **Offline-First Field Reporter (PWA):** Resilient mobile incident reporting app operating without internet and auto-syncing via IndexedDB.
8. **Real-Time GPS Telemetry & Geofencing:** Streams mobile/VTS GPS directly onto the operational risk surface with lookahead hazard warnings.
...
│   ├── features.py             # Feature engineering contracts & column definitions
│   ├── train.py                # 2-Stage model training & temporal evaluation
│   ├── obs_predict.py          # Stage 1 cascade scoring for Stage 2 training
│   ├── explain.py              # Saabas decision-path attribution engine (exact XAI)
│   ├── inference.py            # Batch network inference, what-if scoring, caching
...
├── data/
│   ├── sentinel.db             # Primary SQLite database
│   ├── artifacts/              # Serialized joblib models & evaluation metrics
